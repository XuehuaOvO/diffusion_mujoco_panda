...
#     file = '/root/diffusion_mujoco_panda/collecting_test/collecting_6/x_mpc_idx-103.npy'
#     data = np.load(file)
...
def single_ini_process(initial_guess,initial_state,initial_idx, u_ini_memory, u_random_memory, x_ini_memory, x_random_memory, j_ini_memory, j_random_memory):
        ...
        # save cost 
        cost_sum = np.sum(j_ini_memory)
        cost_path = os.path.join(FOLDER_PATH, f'new_cost_'+ 'idx-' + str(initial_idx.item()) + '.npy')
        print(f'total cost --{cost_sum}')
        np.save(cost_path, cost_sum)

        # x traj save
        x_value = np.array(ini_x_states[1])
        y_value = np.array(ini_x_states[2])
        z_value = np.array(ini_x_states[3])

        x_traj = np.zeros((CONTROL_STEPS, 3))
        x_traj[:,0] = x_value
        x_traj[:,1] = y_value
        x_traj[:,2] = z_value

        x_mpc_path = os.path.join(FOLDER_PATH, f'x_mpc_' + 'idx-' + str(initial_idx.item()) + '.npy')
        np.save(x_mpc_path, x_traj)
