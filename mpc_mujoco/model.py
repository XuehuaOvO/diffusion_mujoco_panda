...
class MPC:
    ...
    def create_mpc(self, model):
        ...
        # mterm = ca.sumsqr(
        #     mujoco.mj_forward(self.data.model, self.data) - model.tvp["target_xpos_states"]
        # )     
        # mterm = ca.sumsqr(
        #     qpos - model.tvp["target_joint_states"]
        # )
        ...
        def tvp_fixed_fun(t_now):
            # Define a fixed target joint position (e.g., a specific configuration)
            fixed_target = np.array([[0], [-0.785], [0], [-2.356], [0], [1.571], [0.785]])  # Example target positions
